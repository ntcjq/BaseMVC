 ...
 *
 */
public class TestTaskForm extends JbpmTestCase implements JbpmUtil{

	@Override
	public void deploy() {
		super.setUp();
		repositoryService.createDeployment().addResourceFromClasspath("jpdl/taskForm.jpdl.xml").deploy();
	}
	...
	public void createInstance() {
		...
		map.put("user", user);
		//创建流程 同时 新增流程变量
		ProcessInstance processInstance = executionService.startProcessInstanceByKey("taskForm",map);
	...
	@Override
	public void getTask() {
		List<Task> tasks = taskService.findPersonalTasks("cjq");
		for(int i = 0;i<tasks.size();i++) {
			Task task = tasks.get(i);
			System.out.println("任务名称："+task.getActivityName()+" 处理人："+task.getAssignee() +" 任务ID:"+task.getId() + " 表单资源：" + task.getFormResourceName());
		}
